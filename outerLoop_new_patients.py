...
vcfLocation = '../../vcf/Qiaseq-new-patients/common-regions/'
allfiles = os.listdir(vcfLocation)


with open('newPatientsFeatureVectorsCommon.pkl', 'rb'  ) as f:
    fdict = pickle.load(f)
...
for root, _, files in os.walk('../../vcf/Qiaseq-new-patients/common-regions'):
    pass
pos2cosmic = dict()
duplicates = dict()
...
vcfLocation = '../../vcf/Qiaseq-new-patients/unique-regions/'
allfiles = os.listdir(vcfLocation)


with open('newPatientsFeatureVectorsUnique.pkl', 'rb'  ) as f:
    fdict = pickle.load(f)
...
uniqueCalled = uniqueRegionCalls[uniqueRegionCalls['model_score_positive'] == 1]
...
zz = np.zeros((uniqueCalled.shape[0], 4))


for i in range(4):
    zz[:, i] = ps[i] * beta.pdf(np.array(uniqueCalled['VAF_2']), alphas[i], betas[i])
...
uniqueCalled['prob_error'] = zz[:,0]
uniqueCalled['prob_g_heterozygous'] = zz[:, 3]
uniqueCalled['prob_g_homozygous'] = zz[:, 1]
uniqueCalled['prob_somatic'] = zz[:, 2]


uniqueCalled['label'] = variantClasses[np.argmax(zz, axis=1)]




for root, _, files in os.walk('../../vcf/Qiaseq-new-patients/unique-regions'):
    pass
pos2cosmic = dict()
duplicates = dict()
...
cosmicIDs = []
for i in range(uniqueCalled.shape[0]):
    p = uniqueCalled.iloc[i, 1] + '-' + str(uniqueCalled.iloc[i, 2]) + '-' + str(uniqueCalled.iloc[i, 3]) + '-' + str(uniqueCalled.iloc[i, 4])

    cosmicIDs.append(pos2cosmic[p])

uniqueCalled['cosmicID'] = cosmicIDs
...
uniqueCalled['dbSNP'] = dbSNPids

uniqueCalledFiltered = uniqueCalled[uniqueCalled['dbSNP'] == '.']
...
for c in uniqueCalledFiltered['cosmicID']:
    try:
        row = icgcInfo.loc[c]
        stats = list(row)
    except KeyError:
...
uniqueCalledFiltered['ICGC-status'] = status
uniqueCalledFiltered['ICGC-FR-count'] = frc
uniqueCalledFiltered['ICGC-FR-%'] = frp
uniqueCalledFiltered['ICGC-EU-count'] = euc
uniqueCalledFiltered['ICGC-EU-%'] = eup
uniqueCalledFiltered['ICGC-US-count'] = usc
uniqueCalledFiltered['ICGC-US-%'] = usp
uniqueCalledFiltered['ICGC-TOTAL-count'] = totc
uniqueCalledFiltered['ICGC-TOTAL-%'] = totp
...
dens_model = gaussian_kde(uniqueCalled['VAF_2'])
ax.plot(xx, dens_model(xx), color='r', label='model')

dens_filter = gaussian_kde(uniqueCalledFiltered['VAF_2'])
...
uniqueCalledFiltered.to_csv('../../new-patients-qiaseq-unique-regions.csv')
commonCalledFiltered.to_csv('../../new-patients-qiaseq-common-regions.csv')
